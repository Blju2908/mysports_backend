...
from app.schemas.training_plan_schemas import TrainingPlanCreate, TrainingPlanRead

router = APIRouter(tags=["training-plan"])


@router.get("/mine", response_model=TrainingPlanRead)
...
@router.put("/mine", response_model=TrainingPlanRead)
async def update_my_training_plan(
    plan_update: TrainingPlanCreate,
    db: AsyncSession = Depends(get_session),
    current_user: User = Depends(get_current_user)
):
    """
    ...
    training_plan = await db.scalar(
        select(TrainingPlan)
        .where(TrainingPlan.user_id == UUID(current_user.id))
    )
    ...
    # Update other scalar fields (excluding id, user_id, birthdate, and training_profiles)
    update_data = plan_update.model_dump(
        ...
        exclude={
            ...
            "user_id",
            "birthdate",
            "equipment", # Exclude old equipment field
            "workout_styles", # Exclude old workout_styles field
        }
    )
